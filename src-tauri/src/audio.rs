...
impl AudioPlayer {
            ...
            for cmd in cmd_rx {
                match cmd {
                    ...
                    AudioCommand::GetDuration { reply } => {
                        let duration = match backend.as_ref() {
                            Some(PlaybackBackend::Normal { pipeline, .. }) => pipeline
                                .query_duration::<gst::ClockTime>()
                                .map(|d| d.nseconds() as f32 / 1_000_000_000.0)
                                .unwrap_or(0.0),
                            Some(PlaybackBackend::DirectAlsa { pipeline, .. }) => pipeline
                                .query_duration::<gst::ClockTime>()
                                .map(|d| d.nseconds() as f32 / 1_000_000_000.0)
                                .unwrap_or(0.0),
                            None => 0.0,
                        };
                        reply.send(Ok(duration)).ok();
                    }
    ...
    pub fn get_duration(&self) -> Result<f32, String> {
        self.send_cmd(|reply| AudioCommand::GetDuration { reply })
    }
